...
class NoTrainInceptionV3(_FeatureExtractorInceptionV3):
    ...
    def _torch_fidelity_forward(self, x: Tensor) -> Tuple[Tensor, ...]:
        ...
        """
        vassert(torch.is_tensor(x), "Expecting image as torch.Tensor") # vassert(torch.is_tensor(x) and x.dtype == torch.uint8, "Expecting image as torch.Tensor with dtype=torch.uint8")
        features = {}
